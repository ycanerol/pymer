...
def splitpars(kQmu):
    ...
    """
    k, mu = np.split(kQmu, [filter_length])
...
def gqm_neuron(k, mu, time_res):
    ...
    """
    def fr(x):
        return np.exp(gqm_in(k, mu)(x))*time_res
    return fr
...
#                           Q_initial,
    ...
    """
    kQmu_initial = flattenpars(k_initial,
#                               Q_initial
                               mu_initial)
...
if __name__ == '__main__':
    ...
    f = gqm_neuron(k_in, mu_in, time_res)
    rate = f(stim)
    ...
    res = minimize_loglikelihood(np.zeros(k_in.shape),
#                                 np.zeros(Q_in.shape),
                                 0,
