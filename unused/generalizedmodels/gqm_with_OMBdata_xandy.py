...
stimulus = stimulus_xy.sum(axis=0)
...
savedir = os.path.join(exp_dir, 'data_analysis', stimname, 'GQM')
...
#    plt.figure(figsize=(12,10))
    axk = plt.subplot(321)
    axk.plot(sta, color='grey', label='STA')
    ...
    axv.plot(w_out, 'ko')
    eiginds = [0, 1, l-1]
    axw.plot(data['eigvecs_x'][i][:, [-1, 0, 1]], '--', color='grey')
...
#    plt.savefig(os.path.join(savedir, clids[i]+'.svg'), bbox_inches = 'tight', pad_inches = 0.3)
    plt.show()
