...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 20 17:48:09 2018

@author: ycan

Duplicate of stripesurround.py
Trying first N SVD components to reduce
noise in stripeflicker STA surround,
perhaps get better fits.

Conclusion:
Reduces the noise in the STA but no
substantial improvement in the surround
fits.
"""
import os
import numpy as np
...
def component(i, u, s, v):
    """
    Get the i-th component of the singular value decomposition
    """
    # s containts singular values (e.g. weights), don't forget it!
    c = np.outer(u[:, i], v[i, :])*s[i]
    return c

def sumcomponent(nr, sta):
    """
    Sum of first nr components of singular value decomposition
    """
    # Perform the calculation here (not in component) to
    # avoid repeatedly calculating SVD.
    u, s, v = np.linalg.svd(sta)
    cumulative = np.zeros(sta.shape)
    # nr = 1 gives the first component.
    for comp in range(nr):
...
def stripesurround_SVD(exp_name, stimnrs, nrcomponents=5):
    ...
    for stimnr in stimnrs:
        ...
        for i in range(clusters.shape[0]):
            ...
            try:
                sta_reduced, max_i = msc.cutstripe(sta_reduced, max_i, fsize*2)
            ...
            if max_i[1] < cut_time:
                max_i[1] = cut_time+1
            fitv = np.mean(sta_reduced[:, max_i[1]-cut_time:max_i[1]+cut_time+1],
                           axis=1)
