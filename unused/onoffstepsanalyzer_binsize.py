...
This was used to determine whether current binsize of 10ms is best,
if it changes onoff index calculation.

Conclusion:
Index stays exactly the same because it's the sum of the number of spikes
in each period. Bin size changes the peaks and the look of the PSTH only.
"""
import os
import numpy as np
...
def onoffstepsanalyzer(exp_name, stim_nrs, tstep=.01):
    """
...
#        tstep = 0.01
        ...
        all_frs = []
        np.random.seed(6)
        index = int(np.random.randint(0, clusters.shape[0], 1))

        clusters = np.array(clusters[index:index+1, :])
        clusterids = clusterids[index]

        for i in range(len(clusters[:, 0])):
            print(i, clusters, clusterids)
